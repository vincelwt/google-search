...
GoogleSearch.prototype.build = function(options, callback) {
  GoogleSearch.prototype._doRequest(GoogleSearch.prototype._generateUrl(options), callback);
};

GoogleSearch.prototype._generateUrl = function(query) {
  query.key = GoogleSearch.prototype.config.key;
  query.cx = GoogleSearch.prototype.config.cx;
  var pathname = GoogleSearch.prototype.config.path;
  //console.log(pathname);
  var urlFormatted = url.format({
    protocol: "https",
    hostname: GoogleSearch.prototype.config.host,
    pathname: pathname,
    query: query
  });
